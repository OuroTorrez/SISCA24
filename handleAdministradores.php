...
if ($_POST['accion'] == "showExistencias") {
    ...
    if($id_almacen != 0) {
        $query = $conn->prepare("SELECT DISTINCT rd.folio, p.nombre, rd.dotacion, DATE_FORMAT(rd.fecha_registro, '%d/%m/%Y %H:%i:%s') AS fecha_registro, d.programa, rd.pdf_docs, rd.cancelado, rd.nota_cancelacion
    ...
    if ($query->execute()) {
        $query->bind_result($folio, $proveedor, $dotacion, $fecha, $programa, $pdf_docs, $activo, $nota_cancelacion);
        $query->store_result();
        if ($query->num_rows > 0) {
            ?>
                            ...
                            ?>
                                ...
                                if ($pdf_docs != null) {
                                    ...
                                    <td class="t-center"><a data-tooltip="Consultar documentos"
                                            onclick="consultarDoc(<?php echo $folio ?>, 'Entradas', <?php echo $_SESSION['rol'] ?>)"><i
                                                class="bi bi-file-earmark-text"></i></a></td>
                                <?php
                                } else if ($pdf_docs == null && ($_SESSION['rol'] != 3 && $_SESSION['rol'] != 4)) {
                                    ?>
                                ...
                                <?php
                                } else if ($_SESSION['rol'] == 4 && $activo == 1) {
                                    ?>
                                        <td class="t-center"><a data-tooltip="Cancelar"
    ...
    $almacen = $_POST['almacen'];
    if($almacen != 0){
        $query = $conn->prepare("SELECT DISTINCT sd.folio, sd.afavor, sd.municipio, sd.dotacion, sd.fecha_registro, sd.pdf_docs, sd.pdf_docs_coord, d.programa, sd.cancelado, sd.nota_cancelacion
    ...
    if ($query->execute()) {
        $query->bind_result($folio, $afavor, $municipio, $dotacion, $fecha, $pdf_docs, $pdf_docs_coord, $programa, $activo, $nota_cancelacion);
        $query->store_result();
        if ($query->num_rows > 0) {
            ?>
            <table id="tablaRegistros" style="padding-bottom: 50px;">
                ...
                <tbody>
                    ...
                    while ($query->fetch()) {
                        ...
                        if($pdf_docs_coord != null && ($_SESSION['rol'] == 3 || $_SESSION['rol'] == 4)){
                            ?>
                            ...
                            <?php
                        } else if ($pdf_docs_coord == null && $_SESSION['rol'] == 3){
                            ?>
                            ...
                            <?php
                        } else if ($pdf_docs_coord == null && $_SESSION['rol'] == 4){
                            ?>
                            <td class="t-center"><a data-tooltip="Sin documentos"><i class="bi bi-file-earmark-x"></i></a></td>
                            <?php
                        }
